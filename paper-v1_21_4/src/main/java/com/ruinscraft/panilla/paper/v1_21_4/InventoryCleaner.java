package com.ruinscraft.panilla.paper.v1_21_4;
        ...
        for (int slot = 0; slot < container.getContents().size(); slot++) {
            ...
            for (FailedNbt failedNbt : failedNbtList) {
                if (FailedNbt.failsThreshold(failedNbt)) {
                    ...
                    DataComponentType<?> type = registry.get(ResourceLocation.parse(failedNbt.key)).map(Holder.Reference::value).orElse(null);
